...
void JESUS_CHRIST_conv(const char* str, BYTE* hex){
...
void JesusChrist_aes_dec(char* Holy_key, char * JESUS_CHRISTcipher, char* Gospel_command)
{
    WORD key_schedule[60];
    BYTE enc_buf[128];
  
    BYTE *key2 = (BYTE*) Holy_key;
    BYTE key[32]; int jc;
    for(jc=0; jc<32; jc++)
    key[jc] = key2[jc];
    aes_key_setup((BYTE*)Holy_key, key_schedule, 256);
    aes_decrypt((BYTE*) JESUS_CHRISTcipher, enc_buf, key_schedule, 256);
     char lord=0;
    int i;
    sprintf(Gospel_command, "%c", hex_to_ascii(enc_buf[0], enc_buf[1]));
    for(i=2; i<strlen((char*)enc_buf); i++){
        if(i%2 != 0) sprintf(Gospel_command , "%s%c", Gospel_command , hex_to_ascii(lord, enc_buf[i]));
            else lord = enc_buf[i];
    }
}

void JesusChrist_aes(char* Holy_key, char* Gospel_command, char* JESUS_CHRIST_aescbc)
{
    WORD key_schedule[60];
    BYTE enc_buf[128];
    BYTE plaintext[32];// =
    ...
    JESUS_CHRIST_conv(Gospel_command, plaintext);
    ...
    for(jc=1; jc<strlen((char*)enc_buf); jc++)
     JESUS_CHRIST_aescbc[jc]=(char) enc_buf[jc];
}

//Generate  JESUS+8-digit random number as salt
void Jesus_salt(char* Christ){
    int Lord, number;
   sprintf(Christ, "%s", "J");
    for(Lord=0; Lord<1; Lord++){
       number = (int) (10.0*rand()/(RAND_MAX + 1.0));
        sprintf(Christ, "%s%d",Christ, number);
    }
}
void Jesus_sha256(char Jesus_pinSalt[], char* Christ_key){
    const char *text1 = Jesus_pinSalt;//[] = "LordJesusChrist";
    BYTE buf[SHA256_BLOCK_SIZE];
    SHA256_CTX ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, (const BYTE*)text1, strlen(text1));
    sha256_final(&ctx, buf);
    int Jesus;
    sprintf(Christ_key, "%2.2x", (unsigned int) buf[0]);
    for(Jesus=1; Jesus<SHA256_BLOCK_SIZE; Jesus++){
        sprintf(Christ_key, "%s%2.2x", Christ_key, (unsigned int) buf[Jesus]);
    }
}
...
int main()
{
    ...
    uBit.init();
    uBit.radio.enable();
    char Christ_pinSalt[32], Heaven_key[80], Heaven_key2[80], Jesus_cypher[160], JESUS_CHRIST_aesecb[160];
    int pin; char *command, JESUS_CHRIST_salt[25], Gods_command[20];
    ManagedString s ="The Lord GOD is good";
    ManagedString t = "Jesus Christ is Lord Amen";
    while(1)
    {
        if (uBit.buttonA.isPressed()){
            ...
            command = "scroll";
            pin = 1;
            Jesus_salt(JESUS_CHRIST_salt); //generate a random salt
            sprintf(Christ_pinSalt, "%d%s", pin, JESUS_CHRIST_salt); //concatenate pin+salt
            Jesus_sha256(Christ_pinSalt, Heaven_key); // Hash with sha256 to get the dpk
            JesusChrist_aes(Heaven_key, command, JESUS_CHRIST_aesecb); // get the AES ecb of the command using key
            sprintf(Jesus_cypher, "%s%s", Christ_pinSalt, JESUS_CHRIST_aesecb); //join salt and cipher into a text for sending
            ManagedString b(JESUS_CHRIST_aesecb);
            int k;
            for(k=0; k<strlen(JESUS_CHRIST_aesecb); k++)
            uBit.serial.printf("%02x",JESUS_CHRIST_aesecb[k]);
            uBit.serial.printf("\nGOD have mercy\n");
            char JesusCh[160];
            sprintf(JesusCh, "%02x", JESUS_CHRIST_aesecb[0]);
            for(k=1; k<strlen(JESUS_CHRIST_aesecb); k++)
            sprintf(JesusCh, "%s%02x",JesusCh, JESUS_CHRIST_aesecb[k]);
             uBit.serial.printf("\n%s", JesusCh);
            //Jesus_sha256(Christ_pinSalt, Heaven_key2);
            //JesusChrist_aes_dec(Heaven_key2, JesusCh, Gods_command);
