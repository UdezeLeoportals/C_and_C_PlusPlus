...
int main()
{
    ...
    BYTE enc_buf[128], enc_buf4[128];
    BYTE plaintext4[32];// ={0x6b,0xc1,0xbe,0xe2,0x2e,0x40,0x9f,0x96,0xe9,0x3d,0x7e,0x11,0x73,0x93,0x17,0x2a};
    
    char Christ_salt[10], GOD_JESUSC_SPIRIT_cyper1[32], GOD_JESUSC_SPIRIT_cyper2[32];
    
    //BYTE ciphertext[2][16] = {
       // {0xf3,0xee,0xd1,0xbd,0xb5,0xd2,0xa0,0x3c,0x06,0x4b,0x5a,0x7e,0x3d,0xb1,0x81,0xf8},
    //};
    
    BYTE key[32];// ={0x6d, 0xdc, 0xde, 0xa2, 0x5b, 0xa3, 0x9e, 0x34, 0x92, 0x32, 0x24, 0x5f, 0x70, 0x25, 0xe5, 0x13, 0x26, 0xa6, 0x1e, 0xaa, 0x37, 0x03, 0xd9, 0x8c, 0xa8, 0x8a, 0x28, 0x9b, 0xc9, 0xa4, 0xd3, 0x66};
     while(1){
       //Generate random salt
       int Lord, number;
       sprintf(Christ_salt, "%s", "J");
        for(Lord=0; Lord<1; Lord++){
           number = (int) (10.0*rand()/(RAND_MAX + 1.0));
            sprintf(Christ_salt, "%s%d",Christ_salt, 5);
        }
        if (uBit.buttonA.isPressed()){
            int pin =1;                         //pin
            char * Gospel_command = "scroll"; //command
            JESUS_CHRIST_conv(Gospel_command, plaintext4);
            char ChristSalt[12];
            sprintf(ChristSalt, "%d%s", pin, Christ_salt); //pin+salt concatenation
            
            //char *plaintext6="1J5";
            //Perform SHA 256 on the pin+salt to get the dp key
            ...
            for(Jesus=0; Jesus<SHA256_BLOCK_SIZE; Jesus++){
                key[Jesus]= buf[Jesus];
            }

            //Perform AES ECB Encryption on the command using the key
            aes_key_setup(key, key_schedule, 256);
            //uBit.serial.printf(  "Key          : ");
            //print_hex(key, 32);

            aes_encrypt(plaintext4, enc_buf, key_schedule, 256);
             //uBit.serial.printf("\nPlaintext    : ");
            //print_hex(plaintext4, strlen((char*)plaintext4));
             //uBit.serial.printf("\n-encrypted to: ");
            //print_hex(enc_buf, 16);
            
           //copy cipher text from enc_buf to Christ C array
           char Christ[160], LordJesusChrist[160];// = (char*)malloc(32*sizeof(unsigned char)); 
          
           sprintf(Christ, "%02x", (unsigned int )enc_buf[0]);
           for(Jesus=1; Jesus<16; Jesus++)
           sprintf(Christ, "%s%02x", Christ, (unsigned int )enc_buf[Jesus]);
            
            //append pin+salt
            sprintf(LordJesusChrist, "%s%s", ChristSalt, Christ);
            
            //split into two strings for radio transmission
               strncpy(GOD_JESUSC_SPIRIT_cyper1, LordJesusChrist, 24);
                GOD_JESUSC_SPIRIT_cyper1[24] = '\0';
        
                strcpy(GOD_JESUSC_SPIRIT_cyper2, &LordJesusChrist[24]);
                
            // send two strings over radio
            ManagedString AlmightyLord1(GOD_JESUSC_SPIRIT_cyper1);
            ManagedString AlmightyLord2(GOD_JESUSC_SPIRIT_cyper2);
            
            uBit.serial.send(AlmightyLord1);
            uBit.serial.send("\n");
            uBit.serial.send(AlmightyLord2);
           //uBit.serial.printf("\n-Cyphertext: %s",GOD_JESUSC_SPIRIT_cyper1);
           //uBit.serial.printf("\n-decrypted to: %s", GOD_JESUSC_SPIRIT_cyper2);
            //uBit.radio.datagram.send(AlmightyLord1);
            //uBit.radio.datagram.send(AlmightyLord2);
          /*  aes_decrypt((BYTE*)enc_buf, enc_buf4, key_schedule, 256);
