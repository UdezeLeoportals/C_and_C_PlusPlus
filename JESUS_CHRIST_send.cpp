...
void JesusChrist_aes(char* Holy_key, char* Gospel_command, char* JESUS_CHRIST_aescbc)
{
   ...
   sprintf(JESUS_CHRIST_aescbc, "%02x", enc_buf[0]);
    for(jc=1; jc<strlen((char*)enc_buf); jc++)
     sprintf(JESUS_CHRIST_aescbc, "%s%02x", JESUS_CHRIST_aescbc, enc_buf[jc]);
}
...
int main()
{
    ...
    uBit.init();
    uBit.radio.enable();
    char Christ_pinSalt[32], Heaven_key[80], Jesus_cypher[160], JESUS_CHRIST_aesecb[160];
    int pin; char *command, JESUS_CHRIST_salt[25];
    ...
    while(1)
    {
        if (uBit.buttonA.isPressed()){
            ...
            printf(Jesus_cypher, "%s%s", Christ_pinSalt, JESUS_CHRIST_aesecb); //join salt and cipher into a text for sending
