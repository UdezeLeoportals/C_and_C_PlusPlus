...
void JESUS_CHRIST_conv(char* str, char* hex){
...
int mercy=0;
char *Christ_cypherSalt, *Christ_, GODly[50];
ManagedString Holy, Spirit;
void onData(MicroBitEvent)
{   mercy++;
    Holy = uBit.radio.datagram.recv();
    Spirit = uBit.radio.datagram.recv(); 
    //uBit.display.scrollAsync("HELLO JESUS");
    Christ_cypherSalt= (char*)Spirit.toCharArray();
    Christ_= (char*)Holy.toCharArray();
    //char GODly[50];
    sprintf(GODly, "%s AND %s", Christ_cypherSalt, Christ_);
    char Christ_pinSalt[32], Heaven_key[100], Gods_command[20], JesusChrist_cypher[200];
    
    strncpy(Christ_pinSalt, Christ_cypherSalt, 3);
    Christ_pinSalt[20] = '\0';
    
    strcpy(JesusChrist_cypher, &Christ_cypherSalt[3]);
    Jesus_sha256(Christ_pinSalt, Heaven_key);
    //if(mercy==3)
     uBit.display.scroll(GODly);
    //uBit.display.scroll(Christ_cypherSalt);
     //if(mercy==2) uBit.display.scroll(GODly);
